...
export function useBookings({ businessId, clientId, date, dateFrom, dateTo, status, archived }: UseBookingsOptions = {}) {
  ...
  const fetchBookings = useCallback(async () => {
    setLoading(true);
    let query = supabase.from('bookings').select('*, services(name, price, image_url, description, duration_minutes), businesses(id, name, settings), professionals(name, avatar_url, specialty), client:profiles!bookings_client_id_fkey(id, full_name, email, phone, avatar_url)')
